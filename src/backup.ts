...
const dumpToFile = async (path: string): Promise<void> => {
  ...
  await new Promise(async(resolve, reject) => {
    const command = `mysqldump --host=${env.BACKUP_DATABASE_HOST} --port=${env.BACKUP_DATABASE_PORT} --user='${env.BACKUP_DATABASE_USER}' --password='${env.BACKUP_DATABASE_PASSWORD}'  --single-transaction --routines --triggers --databases ${env.BACKUP_DATABASE_NAME} | gzip > ${path}`;
